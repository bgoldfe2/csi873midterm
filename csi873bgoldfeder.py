...
class NeuralNet(object):
    ...
    def calcDeltaKH(self,d_k):
        ...
        #print('ah ',self.ah)
        numH = len(self.ah)
        rows,cols = self.wi.shape
        #print ('num rows,cols of d_k ',rows, '-',cols,' shape is ',self.wi.shape)
        ...
        for y in range(cols):
            sum = 0.0
            for x in range(rows):
                #print('w_kh ',self.wi[x,y],' d_k ',d_k[y])
                delta_h2 = self.wi[x,y] * d_k[y]
                sum += delta_h2
                #print(sum)
            #print('full sum ',sum)
...
def main():
    ...
    myNet = NeuralNet(784, 4, 10, 50, 3,lrn_rate=0.3, momentum = 0.3)
